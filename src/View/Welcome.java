...
public class Welcome {
	...
	public Welcome(Database database) {
		...
		panel.add(new JLabel("        Welcome to VPPCOE Connect", 40, GUIConstants.black, Font.BOLD), BorderLayout.NORTH);
		...
		createAcc.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
                ...
                if (!email.getText().endsWith("@pvppcoe.ac.in")) {
                    new Alert("Only @pvppcoe.ac.in email addresses are allowed", frame);
                    return;
                }
				if (password.isEmpty()) {
		...
		JLabel login = new JLabel("<html><u>Already have an account? Login</u></html>", 20,
